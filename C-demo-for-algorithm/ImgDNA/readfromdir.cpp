...
{
    //0:file.jpg   1:path + file  2: jpg  3:file  4:path
    if (type ==0)
    {
    ...
    {
        string::size_type endPosition = path.find_last_of('/');
        string subPath = path.substr(0, endPosition);
        return subPath;
    }
    else
    {
...
void ClassImg(const vector<string >& fullFileNames, std::string saveImagePath)
{
    vector<bool> referFlag(fullFileNames.size(),true);

    string referFileName = saveImagePath + "/refer/";
    string trainFileName = saveImagePath + "/train/";
    ...
    for (int i = 0;i < fullFileNames.size(); ++i)
    {
        ...
        {
            fileName = referFileName + className;
...
//            std::cout<<"write refer name :"<<fileName<<endl;
        ...
        {
            fileName = trainFileName + className;
            mkdir(fileName.c_str(),S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
